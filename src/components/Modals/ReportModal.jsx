...
const ReportModal = ({ showReport, reportAction }) => {
  ...
  const listOfReasons = ["Impersonation", "Fruad or Scam", "Inappropriate Content", "Misinformation"];
      ...
      <div>
        ...
        <div className="d-flex gap-3 flex-wrap my-3">
          {listOfReasons.map((list, key) => {
            return (
              <button className={`reason-btn ${reasons.includes(list) && "clicked"}`} onClick={() => handleButtonClick(list)} key={key}>
                {list}
              </button>
            );
          })}
        ...
        <div>
          ...
          <form onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="text-center mt-5">
              <button className="primary-btn small-btn" onClick={handleInputAdd}>
                Next
              </button>
            </div>
          </form>
    ...
    return (
      <div>
        ...
        <div className="step-two-wrapper">
          <p> We need your answer, so we can better understand what’s going on with this account’s profile.</p>
          <p>
            {" "}
            We take reports seriously. If we find a rule violation, we’ll either ask them to remove the content, temporarily or permanently block the
            account
          </p>
          <p>If there’s immediate danger, call your local emergency services, in addition to reporting.</p>
  ...
  const StepThree = () => {
    return (
      <div className="step-three-wrapper">
        <div className="report-header">You’ve selected the following reason</div>
        <ul>
          {reasons.map((reason, key) => {
            return <li key={key}>{reason}</li>;
          })}
        </ul>
        <div className="text-box-header mt-3">Want to follow the status of your report</div>
        <div className="mt-2 checkbox-wrapper">
          <input value={receiveUpdate} onChange={handleChange} type="checkbox" id="updatesCheckbox" />{" "}
          <label htmlFor="updatesCheckbox">Receive updates on this report</label>
        </div>
  ...
  const FinalStep = () => {
    return (
      <div className="final-step-wrapper">
        ...
        <p> We know it wasn’t easy, so we appreciate you taking the time to make this report.</p>

        <div>
          <div className="text-box-header">What’s happening now</div>
          <p>Your report is in our queue. A member of our team will review your report and we will take neccessary actions.</p>
        </div>
